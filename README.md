<p align="center"><img src="public/logo.png" width="88" alt="分工台标志"></p>
<h1 align="center">分工台 · Model Team</h1>
<p align="center">由主模型判断什么时候、把什么任务交给哪个模型。</p>

分工台是本地运行的多模型协作配置台，可接入 Codex。总指挥自主判断自己完成或按需委派，不把任务固定拆成几个步骤。每次委派前简述「任务 → 模型」，子模型提供建议，主助手负责整合、改文件与验证。
...
- **上下文精简**：两个 MCP 工具，只传任务所需片段，不自动转发完整会话。
...
macOS 可双击 `启动分工台.command`；也可以运行 `npm run open` 启动并打开网页。
