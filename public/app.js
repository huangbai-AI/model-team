...
function dashboard() {
  ...
  return (
    ...
    `<div class="overview-strip"><div><small>默认偏好</small><b>${strategies[c.rules.strategy]}</b></div><div><small>候选模型</small><b>${enabled.length} 个 <span>按需选择</span></b></div><div><small>并行上限</small><b>${c.rules.maxParallel} 个 <span>独立任务</span></b></div><div><small>Codex 接入</small><b>${state.integration.installed ? "配置已写入" : "尚未接入"}</b></div></div><div class="dashboard-grid"><section><div class="card commander-card"><div class="commander-title"><img src="/logo.png" alt="" class="commander-logo"><div><span class="eyebrow">总指挥</span><h2>${esc(c.commander)}</h2></div><span class="subtle-badge">自主判断</span></div><p>理解任务、选择协作者、整合结果并验证。互不依赖的任务可同时交给多个模型。</p><div class="guardrails"><span>不固定步骤</span><span>独立任务并行</span><span>不降低验收标准</span></div></div><div class="section-heading team-heading"><h2>什么时候用哪个模型</h2><span class="small muted">选择参考 · 随实际效果调整</span></div><div class="candidate-list">${enabled.map((m) => `<article class="card candidate-row">${avatar(m)}<div><div class="candidate-heading"><h3>${esc(m.name)}</h3><span class="connection ${m.lastTest?.ok === false ? "bad" : ""}">${health(m)}</span></div><p>${esc(m.hint || "由总指挥根据任务选择")}</p></div><button class="button quiet" data-edit="${m.id}" aria-label="编辑 ${esc(m.name)}">编辑</button></article>`).join("") || '<div class="card panel">尚无启用的候选模型。<button class="button primary" data-add>添加模型</button></div>'}</div></section><aside><div class="card panel quality-panel"><span class="eyebrow">选择原则</span><h2>选择适合这段时间的模式</h2>${strategyRadios()}<p class="small muted">点击即保存，后续分工读取新偏好。</p><div class="help-list"><div><b>先判断是否需要委派</b><p>简单任务直接做，独立子任务才考虑协作。</p></div><div><b>能并行就一起开始</b><p>两个以上互不依赖的任务会一次并行分配，缩短等待。</p></div><div><b>结果必须过关</b><p>总指挥整合并验证；不达标就换模型或接手。</p></div></div><details class="policy-details"><summary>查看已保存的完整偏好</summary><p>${esc(c.rules.policy)}</p></details></div><div class="notice mt">${icon("shield")}<div><b>团队共享用成员令牌</b>真实 key 留在团队网关，成员令牌可单独撤销和限额。<br><a href="https://github.com/huangbai-AI/model-team/tree/main/team-gateway" target="_blank" rel="noreferrer">查看部署模板 →</a></div></div><div class="notice mt">${icon("file")}<div><b>调用前，简述全部分工</b>任务 A → 模型；任务 B → 模型。只传必要上下文。</div></div></aside></div>`
  );
}
function planHTML(p) {
...
function models() {
  return (
    ...
    (state.config.models.length
      ? `<div class="model-grid">${state.config.models.map((m) => `<article class="card model-card ${m.enabled ? "" : "disabled"}"><div class="model-card-head">${avatar(m)}<div><h3>${esc(m.name)}</h3><p>${tiers[m.tier]} · ${m.enabled ? "已启用" : "已停用"}</p></div></div><div class="model-id">${esc(m.model)}</div><p class="model-hint">${esc(m.hint || "可添加适用任务，供总指挥参考。")}</p><details class="connection-details"><summary>连接信息</summary><p>${esc(m.baseUrl || "使用 Codex 已有登录")}</p></details><div class="model-details"><div><span>协议</span><span>${m.kind === "codex" ? "Codex 已有登录" : m.kind === "gateway" ? "团队网关令牌" : m.kind === "codex-provider" ? "独立 Codex 通道" : esc(state.config.protocols.find((p) => p.id === m.protocol)?.label || m.protocol)}</span></div><div><span>密钥</span><span>${m.hasKey ? (m.kind === "gateway" ? "成员令牌已保存" : "已加密保存") : "未设置 / 本地免密"}</span></div><div><span>输入 / 输出（每百万词元）</span><span>${m.inputPrice == null ? "未计价" : "$" + m.inputPrice} / ${m.outputPrice == null ? "未计价" : "$" + m.outputPrice}</span></div></div><div class="model-actions"><button class="button secondary" data-test="${m.id}">测试连接</button><button class="button secondary" data-edit="${m.id}">编辑</button><button class="button quiet danger" data-delete="${m.id}">删除</button></div><div id="test-${m.id}" class="test-result">${m.lastTest ? (m.lastTest.ok ? "最近测试：连接成功" : "最近测试：" + esc(m.lastTest.error)) : "尚未测试"}</div></article>`).join("")}</div>`
...
function connect() {
  ...
  return (
    heading(
      "连接你的工作流",
      "继续在 Codex 里工作",
      "合流管理模型与规则，Codex 负责理解项目、并行分工和验证结果。",
    ) +
...
function setKind() {
  ...
  const gateway = f.elements.kind.value === "gateway";
  f.elements.baseUrl.required = !native;
  f.elements.baseUrl.disabled = native;
  f.elements.apiKey.disabled = native;
  if (gateway) f.elements.protocol.value = "openai";
  f.elements.protocol.disabled = !["api"].includes(f.elements.kind.value);
  $("#key-hint").textContent = gateway
    ? "填写管理员发给你的成员令牌。真实 key 不应出现在成员电脑上。"
    : "保存后不回显。编辑时留空可保留原密钥。";
}
